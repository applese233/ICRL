...
class RayPPOTrainer(object):
    ...
    def _validate(self):
        ...
        Also collects sample information for wandb logging.
        """
        import torch
        reward_tensor_lst = []
        data_source_lst = []
        all_collected_samples = []  # For wandb logging
        total_accuracy = 0.0
        total_format = 0.0
        total_score = 0.0
        ...
        if not self.config.do_search:
            for test_data in self.val_dataloader:
                ...
                # evaluate using reward_function with sample collection
                # Collect samples only if we haven't reached 10 yet
                need_samples = len(all_collected_samples) < 10
                if need_samples and hasattr(self.val_reward_fn, '__call__'):
                    try:
                        reward_tensor, samples, metrics = self.val_reward_fn(
                            test_batch, collect_samples=True, max_samples=10 - len(all_collected_samples)
                        )
                        all_collected_samples.extend(samples)
                        total_accuracy += metrics.get('avg_accuracy', 0) * len(test_batch)
                        total_format += metrics.get('avg_format_score', 0) * len(test_batch)
                        total_score += metrics.get('avg_total_score', 0) * len(test_batch)
                        total_search_count += metrics.get('total_search_count', 0)
                        total_think_count += metrics.get('total_think_count', 0)
        ...
        if total_samples > 0:
            metric_dict['val/avg_accuracy'] = total_accuracy / total_samples
            metric_dict['val/avg_format_score'] = total_format / total_samples
            metric_dict['val/avg_total_score'] = total_score / total_samples
            metric_dict['val/avg_search_count'] = total_search_count / total_samples
            metric_dict['val/avg_think_count'] = total_think_count / total_samples
            metric_dict['val/total_search_count'] = total_search_count
            metric_dict['val/total_think_count'] = total_think_count
        
        # Log samples to wandb as a table
        if all_collected_samples and 'wandb' in self.logger.logger:
            import wandb
            # Create a wandb table for samples
            columns = ['data_source', 'ground_truth', 'extracted_answer', 'accuracy', 'format_score', 'total_score', 'prompt', 'response']
            table_data = []
            for sample in all_collected_samples[:10]:  # Limit to 10 samples
                table_data.append([
                    sample.get('data_source', ''),
                    sample.get('ground_truth', ''),
                    sample.get('extracted_answer', ''),
                    sample.get('accuracy', 0),
                    sample.get('format_score', 0),
                    sample.get('total_score', 0),
                    sample.get('prompt', ''),  # Truncate for display
                    sample.get('response', ''),  # Truncate for display
                ])
            
            val_table = wandb.Table(columns=columns, data=table_data)
            self.logger.logger['wandb'].log({f'val/samples_step_{self.global_steps}': val_table}, step=self.global_steps)
